...
int fifo_put(fifo_t *fifo, uint8_t *data, uint16_t len)
{
    if(fifo->is_subview)
        return EINVAL;
...
int fifo_put_byte(fifo_t* fifo, uint8_t byte)
{
    return fifo_put(fifo, &byte, 1);
}

static int check_len(fifo_t* fifo, uint16_t len) {
...
int fifo_skip(fifo_t* fifo, uint16_t len) {
  int err = check_len(fifo, len);
  if(err != SUCCESS)
    return err;
...
int fifo_peek(fifo_t* fifo, uint8_t* buffer, uint16_t offset, uint16_t len) {
  int err = check_len(fifo, len);
  if(err != SUCCESS)
    return err;
...
int fifo_pop(fifo_t* fifo, uint8_t* buffer, uint16_t len) {
  // use peek logic to retrieve data
  int err = fifo_peek(fifo, buffer, 0, len);
